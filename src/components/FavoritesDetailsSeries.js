...
import FavSeries, { newNoteFavSS, noteForSUDSS } from "./FavSeries";
...
const FavoritesDetailsSeries = (props) => {
    ...
    const handleOpenS = () => {
        setFormS(noteForSUDSS.note_details)
        setOpenS(true)
    };
    ...
    const handleUpdateNote = (event) => {
        ...
        axios.put(`https://erhanba-71679337ef80.herokuapp.com/api/notes/${noteForSUDSS.note_id}`, newNote)
            .then((res) => {
    ...
    const handleDeleteNote = (event) => {
        ...
        axios.delete(`https://erhanba-71679337ef80.herokuapp.com/api/notes/${noteForSUDSS.note_id}`)
            .then((res) => {
                ...
                fav.length === 0 ?
                    ...
                    <table className="table-favs">
                        ...
                        </thead>
                        {
                            fav.map((favo) => {
                                return (
                                    <FavSeries openMoral={handleOpen} openMoralS={handleOpenS} toast={showToastMessage} favos={favo} />
                                )
                            })
                        }
                    </table>
